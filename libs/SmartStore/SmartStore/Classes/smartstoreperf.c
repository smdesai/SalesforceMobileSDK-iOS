...
// Generated from perf.sql:24

/*
CREATE PROC add_marker (time_ LONG_INT NOT NULL, marker_ TEXT NOT NULL, payload_size_ INTEGER NOT NULL, duration_ INTEGER NOT NULL)
BEGIN
  INSERT INTO perf(time, marker, payload_size, duration) VALUES(time_, marker_, payload_size_, duration_);
END;
*/

#undef _PROC_
#define _PROC_ "add_marker"
CQL_WARN_UNUSED cql_code add_marker(sqlite3 *_Nonnull _db_, cql_int64 time_, cql_string_ref _Nonnull marker_, cql_int32 payload_size_, cql_int32 duration_) {
  cql_code _rc_ = SQLITE_OK;
  sqlite3_stmt *_temp_stmt = NULL;

  _rc_ = cql_prepare(_db_, &_temp_stmt,
    "INSERT INTO perf(time, marker, payload_size, duration) VALUES(?, ?, ?, ?)");
  cql_multibind(&_rc_, _db_, &_temp_stmt, 4,
...
BEGIN
  ...
  LOOP FETCH C
  BEGIN
    CALL printf("%lld,%s,%d,%d\n", C.time, C.marker, C.payload_size, C.duration);
  END;
  CLOSE C;
END;
*/
...
CQL_WARN_UNUSED cql_code print_perf(sqlite3 *_Nonnull _db_) {
  ...
  _rc_ = cql_prepare(_db_, &C,
    "SELECT time, marker, payload_size, duration "
      "FROM perf "
    "ORDER BY time ASC");
  if (_rc_ != SQLITE_OK) { cql_error_trace(); goto cql_cleanup; }
  for (;;) {
    ...
    printf("%lld,%s,%d,%d\n", C_.time, _cstr_1, C_.payload_size, C_.duration);
...
static cql_uint16 dump_perf_col_offsets[] = { 4,
...
// Generated from perf.sql:45

/*
CREATE PROC dump_perf_average ()
BEGIN
  SELECT time, marker, payload_size, avg(duration)
    FROM perf
...
static cql_uint16 dump_perf_average_col_offsets[] = { 4,
